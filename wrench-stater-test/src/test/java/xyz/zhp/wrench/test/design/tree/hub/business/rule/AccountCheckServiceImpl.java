 ...
 */
@Slf4j
@Service(SeckillBeanConstant.accountCheck)
public class AccountCheckServiceImpl extends AbstractSeckillRuleSupport {

    @Override
    public void handle(SeckillReq secKillReq, SeckillResp s, Void dynamicContext) {

        log.info("是否是封禁账号，检测中");
    }

    @Override
    public Boolean match(OgnlContext ognlContext, String ognl, Void unused) {
        ognlContext.put("account_check", new Random().nextBoolean());
